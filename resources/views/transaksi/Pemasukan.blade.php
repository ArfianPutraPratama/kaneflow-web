...
<head>
    ...
    <!-- Custom CSS for dropdown, image preview, and date picker -->
    <style>
        .custom-dropdown {
            position: relative;
            width: 100%;
        ...
        /* Style for date picker with icon */
        ...
        /* Style untuk pesan JavaScript dinonaktifkan */
        .js-disabled-message {
            display: none;
            color: #e74a3b;
            font-weight: bold;
    ...
    <div id="wrapper">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <!-- Pesan jika JavaScript dinonaktifkan -->
                            <noscript>
                                <div class="js-disabled-message">
                                    JavaScript diperlukan untuk menggunakan form ini. Silakan aktifkan JavaScript di
                                    browser Anda.
                                </div>
                            </noscript>
                            ...
                            <form action="{{ route('pemasukan.store') }}" method="post" enctype="multipart/form-data"
                                class="needs-validation" novalidate="" id="pemasukanForm">
                                @csrf
                                ...
                                <div class="form-group pt-3">
                                    <input type="submit" name="simpan" value="Simpan"
                                        class="btn btn-simpan btn-rounded mr-2" id="submitButton" disabled>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Aktifkan tombol submit hanya jika JavaScript aktif
                ...
                const datePicker = flatpickr('.date-picker', {
                    ...
                    maxDate: 'today',
                    disableMobile: true,
                    allowInput: false, // Tidak boleh input manual
                    ...
                    onChange: function(selectedDates, dateStr, instance) {
                        console.log('Tanggal dipilih:', dateStr); // Debugging
                        // Validasi format tanggal saat dipilih
                        if (!dateStr.match(/^\d{2}-\d{2}-\d{4}$/)) {
                            instance.setDate(new Date());
                            alert('Format tanggal tidak valid. Harus DD-MM-YYYY (contoh: 31-12-2024).');
                            return;
                        }
                        // Validasi apakah tanggal valid
                        const [day, month, year] = dateStr.split('-').map(Number);
                        const date = new Date(year, month - 1, day);
                        if (isNaN(date.getTime()) || date.getDate() !== day || date.getMonth() + 1 !==
                            month) {
                            instance.setDate(new Date());
                            alert('Tanggal tidak valid (contoh: 32-12-2024 tidak diperbolehkan).');
                        }
                    },
                    onClose: function(selectedDates, dateStr, instance) {
                        console.log('Tanggal setelah picker ditutup:', dateStr); // Debugging
                        // Pastikan format tetap DD-MM-YYYY saat picker ditutup
                        if (!dateStr.match(/^\d{2}-\d{2}-\d{4}$/)) {
                            instance.setDate(new Date());
                            console.log('Tanggal direset karena format salah:', instance.element.value);
                        }
                    }
                });
                ...
                $('#pemasukanForm').on('submit', function(e) {
                    const tanggalInput = $('.date-picker');
                    let tanggalValue = tanggalInput.val().trim();

                    // Sanitize input: remove any non-date characters (only allow digits and hyphens)
                    tanggalValue = tanggalValue.replace(/[^0-9-]/g, '');

                    console.log('Tanggal sebelum sanitasi:', tanggalInput.val()); // Debugging
                    console.log('Tanggal setelah sanitasi:', tanggalValue); // Debugging

                    // Validasi apakah tanggal kosong
                    if (!tanggalValue || tanggalValue === '') {
                        e.preventDefault();
                        tanggalInput.addClass('is-invalid');
                        tanggalInput.nextAll('.invalid-feedback').text('Tanggal tidak boleh kosong.');
                        return;
                    }

                    // Validasi format tanggal
                    if (!tanggalValue.match(/^\d{2}-\d{2}-\d{4}$/)) {
                        e.preventDefault();
                        tanggalInput.addClass('is-invalid');
                        tanggalInput.nextAll('.invalid-feedback').text(
                            'Format tanggal harus DD-MM-YYYY (contoh: 31-12-2024).');
                        return;
                    }

                    // Validasi apakah tanggal valid
                    const [day, month, year] = tanggalValue.split('-').map(Number);
                    const date = new Date(year, month - 1, day);
                    if (isNaN(date.getTime()) || date.getDate() !== day || date.getMonth() + 1 !== month) {
                        e.preventDefault();
                        tanggalInput.addClass('is-invalid');
                        tanggalInput.nextAll('.invalid-feedback').text(
                            'Tanggal tidak valid (contoh: 32-12-2024 tidak diperbolehkan).');
                        return;
                    }

                    // Rebuild the date string to ensure exact format (DD-MM-YYYY)
                    tanggalValue =
                        `${day.toString().padStart(2, '0')}-${month.toString().padStart(2, '0')}-${year}`;
                    tanggalInput.val(tanggalValue);
                    console.log('Tanggal final sebelum submit:', tanggalValue); // Debugging
