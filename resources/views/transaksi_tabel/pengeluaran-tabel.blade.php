...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <a href="{{ route('Pengeluaran') }}" class="btn btn-entri-data btn-rounded">
                    ...
                    @else
                        <div class="notification" id="successNotification">
                            <span class="icon">✔</span>
                            <span id="notificationMessage">Sukses! Data pengeluaran berhasil dihapus.</span>
                            <span class="close-btn" onclick="hideNotification('successNotification')">✖</span>
                        </div>
                    @endif
                    <!-- Error Notification -->
                                            ...
                                            <table class="table table-bordered dataTable no-footer" id="dataTable"
                                                ...
                                                <thead>
                                                    <tr role="row">
                                                        ...
                                                        <th class="text-center sorting" tabindex="0"
                                                            aria-controls="dataTable" rowspan="1" colspan="1"
                                                            aria-label="Kategori Pengeluaran: aktifkan untuk mengurutkan kolom secara ascending"
                                                            style="width: 150.933px;">Kategori Pengeluaran
                                                        </th>
                                                        <th class="text-center sorting" tabindex="0"
                                                            aria-controls="dataTable" rowspan="1" colspan="1"
                                                            aria-label="Deskripsi Transaksi: aktifkan untuk mengurutkan kolom secara ascending"
                                                            style="width: 227.933px;">Deskripsi Transaksi
                                                        </th>
                                                        <th class="text-center sorting" tabindex="0"
                                                            aria-controls="dataTable" rowspan="1" colspan="1"
                                                ...
                                                <tbody>
                                                    @forelse ($pengeluaranData as $index => $pengeluaran)
                                                        <tr class="{{ $index % 2 == 0 ? 'odd' : 'even' }}"
                                                            ...
                                                            <td width="170">
                                                                {{ $pengeluaran['kategori'] }}</td>
                                                            <td width="250">
                                                                {{ $pengeluaran['deskripsi'] }}</td>
                                                            <td width="120" class="text-right">Rp.
                                                                {{ number_format($pengeluaran['jumlah'], 0, ',', '.') }}
                                                            </td>
                                                            <td width="100" class="text-center">
                                                                <div>
                                                                    <a href="{{ route('pengeluaran.detail', $pengeluaran['id']) }}"
                                                                        class="btn btn-info btn-circle btn-sm mr-md-1"
                                                                        data-toggle="tooltip" data-placement="top"
                                                                        title="Detail">
                                                                        <i class="fas fa-clone"></i>
                                                                    </a>
                                                                    <a href="{{ route('pengeluaran.ubah', $pengeluaran['id']) }}"
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 col-md-5">
                                            <div class="dataTables_info" id="dataTable_info" role="status"
                                                aria-live="polite">
                                                Menampilkan 1 sampai {{ min(count($pengeluaranData), 10) }} dari
                                                {{ count($pengeluaranData) }} data
                                            </div>
                                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            const updatedPengeluaran = sessionStorage.getItem('updatedPengeluaran');
            if (updatedPengeluaran) {
                const data = JSON.parse(updatedPengeluaran);
                updateTableRow(data);
                showNotification('successNotification', 'Sukses! Data pengeluaran berhasil diperbarui.');
                sessionStorage.removeItem('updatedPengeluaran');
            }
            ...
            $(document).on('click', '.delete-btn', function(e) {
                ...
                const message =
                    `Anda yakin ingin menghapus data pengeluaran ${kategori} tanggal ${tanggal} ?`;
                ...
                $('#confirmDeleteBtn').off('click').on('click', function() {
                    ...
                    $.ajax({
                        url: '{{ route('pengeluaran.delete', ['id' => ':id']) }}'.replace(
                            ':id', id),
                        type: 'POST', // Laravel requires POST for form submissions, but we spoof DELETE
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}',
